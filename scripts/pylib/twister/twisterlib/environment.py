...
Artificially long but functional example:
   ...
   "kernel.fifo.poll" is one of the test section names in
    ...
    parser.add_argument("--port", default=None,
                        help="Serial port for Realtek Ameba boards "
                             "(local: /dev/ttyUSB0, remote: COM5).")

    parser.add_argument("--remote-server", default=None,
                        help="Remote serial server IP for Realtek Ameba boards. "
                             "Use together with --port.")
...
def parse_arguments(
    ...
    if (
        ...
        logger.error(
            "Use --device-testing with --device-serial, or --device-serial-pty, "
            "or --hardware-map, or --port."
        )
        sys.exit(1)

    if options.device_testing and options.port and len(options.platform) < 1:
        logger.error("When --device-testing is used with --port, "
