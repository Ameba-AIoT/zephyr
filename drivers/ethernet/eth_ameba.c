...
#define CONFIG_ETH_DMA_RX_BUFFER_NUM 1
#define CONFIG_ETH_DMA_BUFFER_SIZE   1
#define CONFIG_ETH_DMA_TX_BUFFER_NUM 1
#endif
...
#else
	if (esp_read_mac(mac_addr, AMEBA_MAC_ETH) != RTK_OK) {
		res = -EIO;
	}
#endif
	return res;
...
/*static struct eth_ameba_dma_data eth_ameba_dma_data;*/

static struct eth_ameba_dev_data eth_ameba_dev = {
	/*.dma = &eth_ameba_dma_data,*/
};

ETH_NET_DEVICE_DT_INST_DEFINE(0, eth_ameba_initialize, NULL, &eth_ameba_dev, NULL,
			      CONFIG_ETH_INIT_PRIORITY, &eth_ameba_api, NET_ETH_MTU);
