...
#define AMEBA_LCDC_TODO 0

static int ameba_lcdc_write(const struct device *dev, const uint16_t x, const uint16_t y,
			    const struct display_buffer_descriptor *desc, const void *buf)
{
#if AMEBA_LCDC_TODO
	const struct ameba_lcdc_config *config = dev->config;
	struct ameba_lcdc_data *dev_data = dev->data;
	int h_idx;
	const uint8_t *src;
	uint8_t *dst;
	int ret = 0;
	...
	if ((x == 0) && (y == 0) && (desc->width == config->rgb_mode.panelWidth) &&
		...
		full_fb = true;
		   ...
		   IS_ENABLED(CONFIG_AMEBA_LCDC_PXP)) {
	...
	dev_data->next_idx = (dev_data->next_idx + 1) % CONFIG_AMEBA_LCDC_FB_NUM;
#endif
...
static int ameba_lcdc_display_blanking_off(const struct device *dev)
{
	const struct ameba_lcdc_config *config = dev->config;
...
static int ameba_lcdc_display_blanking_on(const struct device *dev)
{
	const struct ameba_lcdc_config *config = dev->config;
...
static int ameba_lcdc_set_pixel_format(const struct device *dev,
				       const enum display_pixel_format pixel_format)
{
	...
	for (int i = 0; i < CONFIG_AMEBA_LCDC_FB_NUM; i++) {
...
static int ameba_lcdc_set_orientation(const struct device *dev,
...
static void ameba_lcdc_get_capabilities(const struct device *dev,
					struct display_capabilities *capabilities)
{
	...
	capabilities->current_pixel_format = ((struct ameba_lcdc_data *)dev->data)->pixel_format;
	capabilities->current_orientation = DISPLAY_ORIENTATION_NORMAL;
#endif
}

static void ameba_lcdc_isr(const struct device *dev)
{
#if AMEBA_LCDC_TODO
	const struct ameba_lcdc_config *config = dev->config;
	struct ameba_lcdc_data *dev_data = dev->data;
...
static int ameba_lcdc_init(const struct device *dev)
{
#if AMEBA_LCDC_TODO
	const struct ameba_lcdc_config *config = dev->config;
	struct ameba_lcdc_data *dev_data = dev->data;
	...
	ameba_lcdc_set_pixel_format(dev, dev_data->pixel_format);
...
static DEVICE_API(display, ameba_lcdc_api) = {
	.blanking_on = ameba_lcdc_display_blanking_on,
	.blanking_off = ameba_lcdc_display_blanking_off,
	.write = ameba_lcdc_write,
	.get_capabilities = ameba_lcdc_get_capabilities,
	.set_pixel_format = ameba_lcdc_set_pixel_format,
	.set_orientation = ameba_lcdc_set_orientation,
};
...
/*.rgb_mode =*/
{
	...
	.dataBus = LCDIF_CTRL_LCD_DATABUS_WIDTH(DT_INST_ENUM_IDX(id, data_bus_width)),
},

/*.backlight_gpio = GPIO_DT_SPEC_INST_GET(id, backlight_gpios), */
#endif

#define AMEBA_LCDC_DEVICE_INIT(id)                                                                 \
	PINCTRL_DT_INST_DEFINE(id);                                                                \
	static void ameba_lcdc_config_func_##id(const struct device *dev);                         \
	static const struct ameba_lcdc_config ameba_lcdc_config_##id = {                           \
		/*.base = (LCDIF_Type *)DT_INST_REG_ADDR(id),*/                                    \
		.irq_config_func = ameba_lcdc_config_func_##id,                                    \
		.pincfg = PINCTRL_DT_INST_DEV_CONFIG_GET(id),                                      \
	};                                                                                         \
	static struct ameba_lcdc_data ameba_lcdc_data_##id = {                                     \
		.next_idx = 0,                                                                     \
		.pixel_format = DT_INST_PROP(id, pixel_format),                                    \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(id, &ameba_lcdc_init, NULL, &ameba_lcdc_data_##id,                   \
			      &ameba_lcdc_config_##id, POST_KERNEL, CONFIG_DISPLAY_INIT_PRIORITY,  \
			      &ameba_lcdc_api);                                                    \
	static void ameba_lcdc_config_func_##id(const struct device *dev)                          \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(id), DT_INST_IRQ(id, priority), ameba_lcdc_isr,           \
			    DEVICE_DT_INST_GET(id), 0);                                            \
		irq_enable(DT_INST_IRQN(id));                                                      \
	}

	DT_INST_FOREACH_STATUS_OKAY(AMEBA_LCDC_DEVICE_INIT)
